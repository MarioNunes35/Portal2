...
# ------------ Utils mínimos (não chamar st.login aqui) ------------
def _get_user_obj():
...
def go_to_login():
    """Voltar para a página principal do app via switch_page.
...
# --------------------------- UI --------------------------------------
def app_card(title: str, description: str, href: str = "", internal_page: str = "", key: str = ""):
    with st.container(border=True):
        ...
        with col1:
            if internal_page:
                # Se for uma página interna deste mesmo projeto
                try:
                ...
                st.button("Indisponível", disabled=True, use_container_width=True, key=f"disabled_{key}")
